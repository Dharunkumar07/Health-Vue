...
class Slide:
    ...
    def enhance(self, img_bgr, sat=1.0):
        ...
        # Resize cached masks to match this image's size
        # (needed when DEFAULT mask from a different slide is used)
        h_img, w_img = x.shape[2], x.shape[3]
        def _rsz(t):
            if t.shape[2] != h_img or t.shape[3] != w_img:
                return F.interpolate(t, size=(h_img,w_img), mode='bilinear', align_corners=False)
            return t
        fill_t = _rsz(self.fill_t)
        bord_t = _rsz(self.bord_t)
        plt_t  = _rsz(self.plt_t)
        wbc_t  = _rsz(self.wbc_t)
        if self.has_ref:
            ycc = _bgr2ycc(x)
...
def get_slide(name, img):
    ...
    if Path(mp).exists():
        ...
        # Use DEFAULT mask with positional tensors (same as RTX quality)
        # Note: for different slides, borders may be slightly off
        # Run /warmup/{name} once per slide for perfect quality
        s.load(DEFAULT_MASK, img, _refs)
        log.info(f"Using DEFAULT mask for '{name}' (run /warmup/{name} for perfect quality)")
